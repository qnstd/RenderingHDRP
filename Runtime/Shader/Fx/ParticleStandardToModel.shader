...
*/
Shader "Graphi/Fx/ParticleStandardToModel"
{
    Properties
    {
        //基本项
...
SubShader
    {
        ...
        Pass
        {
            ...
            #define ATTRIBUTES_NEED_TEXCOORD0 //UV0
...
float _DissolveTex_U, _DissolveTex_V, _DissolveEdgeIntensity, _DissolveIntensity, _DissArea;
//扭曲参数
    ...
    inout float2 uvMain, inout float2 uvMask, inout float2 uvDissolve)
{
    ...
    //计算扭曲值
    twist = (twist * 2 - 1) * _TwistIntensity; //扭曲范围 [-1,1] * _TwistIntensity
...
void Dissolve(inout float4 c, float2 uv, FragInputs fragInputs)
{
    ...
    float factor = _DissolveIntensity; ///溶解系数（0：不溶解；1：完全溶解）
...
// 顶点变换（顶点动画）操作处理函数
             ...
             {
                 //修改顶点信息
                 float2 uvAni = TRANSFORM_TEX(input.uv0.xy, _VertexAniTex);
...
// 片段渲染时，自定义处理操作函数
            ...
            {
                //参数准备
                float2 uv = fragInputs.texCoord0.xy; //主UV
...
// 执行渲染
    ...
    CustomEditor "com.graphi.renderhdrp.editor.ParticleStandardToModelEditorShaderGUI"
}