#ifndef LW_TESSELLATION
#define LW_TESSELLATION
...
/*
    细分操作函数
*/
HsConstantOutput HullConst(InputPatch<TesselationMesh, 3> i)
{
    HsConstantOutput o = (HsConstantOutput)0;

    // 计算每个面的3条边的细分数及内部的细分数
...
TesselationMesh Hull(InputPatch<TesselationMesh, 3> input, uint id : SV_OutputControlPointID)
{
    return input[id];
}
...
*/
[domain("tri")]
TesselationMesh Domain(HsConstantOutput hsConst,  const OutputPatch<TesselationMesh, 3> i, float3 bary : SV_DomainLocation)
{
    UNITY_SETUP_INSTANCE_ID(patch[0]);

    // 创建新增的顶点
    TesselationMesh data = (TesselationMesh)0;
    ...
    // uv
    #define CalculateUV(prop) data.prop = i[0].prop * fW + i[1].prop * fU + i[2].prop * fV;
